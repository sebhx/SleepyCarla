...
import { Calendar, Moon, Bed, Edit, Trash2, Info, X } from "lucide-vue-next";
import {
  formatTime,
...
const allActivities = computed(() => {
  ...
  // Apply type filter (for SleepSession, we can filter by sleepType)
  if (activitiesFilter.value === "sleep") {
    // Show all sleep sessions (no filtering needed)
  } else if (activitiesFilter.value === "wake") {
    // For wake events, we'd need to show sessions that have ended
    filteredEntries = filteredEntries.filter((entry) => entry.endTime);
  }
  ...
  switch (activitiesDateRange.value) {
    ...
    case "month": {
      ...
      filteredEntries = filteredEntries.filter(
        (entry) => entry.startTime >= monthAgo
      );
      break;
    }
    case "all":
  ...
  return filteredEntries
    ...
    .map((entry) => {
      ...
      return {
        ...entry,
        timeFormatted: formatTime(entry.startTime),
        dateFormatted: formatLocalizedDateShort(entry.startTime),
        durationFormatted,
      };
    });
});
...
const handleEdit = (entry: SleepSession) => {
            ...
            >
              <div class="activity-card-header">
                <div class="activity-icon">
                  <Bed
                    v-if="entry.sleepType === 'night'"
                    :size="20"
                    :color="'var(--baby-purple)'"
                  />
                  <Moon v-else :size="20" :color="'var(--baby-blue)'" />
                </div>

                <div class="activity-type">
                  {{ entry.sleepType === "night" ? "Night Sleep" : "Nap" }}
