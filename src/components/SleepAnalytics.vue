...
import { useSleepAnalytics } from "../composables/sleepAnalytics";
...
const showDataPoint = (point: any, type: "sleep" | "nap") => {
...
const overviewChartData = computed(() => {
  ...
  const bars = patterns.map((pattern, index) => {
    ...
    return {
      ...
      pattern,
      label: pattern.date.toLocaleDateString("en-US", { weekday: "short" }),
    };
  });
      ...
      >
        <div class="data-point-content" @click.stop>
          <div class="data-point-header">
            <h3>
              {{
                selectedDataPoint.date.toLocaleDateString("en-US", {
                  weekday: "long",
                  month: "long",
                  day: "numeric",
                })
